...
numCUE = 20;
numDUE = 20;
sumRate_maxSum = zeros(length(dB_gamma0), 1);
minRate_maxSum = zeros(length(dB_gamma0), 1);
sumRate_maxMin = zeros(length(dB_gamma0), 1);
...
sumRate_maxSum2 = zeros(length(dB_gamma0), 1);
minRate_maxSum2 = zeros(length(dB_gamma0), 1);
...
MarkerSize = 6;
figure
plot(dB_gamma0, sumRate_maxSum, 'k-s', 'LineWidth', LineWidth, 'MarkerSize', MarkerSize)
hold on
...
xlabel('$r_{0}^d$ (dB)', 'interpreter','latex')
ylabel('$\sum\limits_m C_m$ (bps/Hz)', 'interpreter','latex')
% saveas(gcf, sprintf('sumRateVsSpeed')); % save current figure to file

figure
plot(dB_gamma0, minRate_maxSum, 'k-s', 'LineWidth', LineWidth, 'MarkerSize', MarkerSize)
hold on
